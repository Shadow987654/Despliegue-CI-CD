...
print_step "Verificando instalacion de Docker..."
...
print_step "Verificando que Docker esta ejecutandose..."
if docker info &> /dev/null; then
    print_success "Docker esta ejecutandose correctamente"
else
    print_error "Docker esta instalado pero no se esta ejecutando"
...
print_step "Verificando archivos de configuracion esenciales..."
    ...
    if [ -f "$file" ]; then
        ...
        case $file in
            ...
            "secrets/flask_secret_key.txt")
                ...
                chmod 600 "$file"
                print_success "Creado: $file (secret key generada automÃ¡ticamente)"
                echo ""
                ;;
            *)
                ...
                if [ -f "$example_file" ]; then
                    ...
                    # Crear archivo basico si no existe plantilla
                    mkdir -p "$dir"
                    touch "$file"
                    print_success "Creado: $file (vacio)"
                fi
                ;;
        esac
    fi
...
if docker-compose down --remove-orphans &> /dev/null; then
    ...
    print_info "No habia contenedores ejecutandose"
fi

# Opcion de limpieza
echo ""
read -p "¿Deseas limpiar imagenes Docker antiguas? (S/N): " cleanup
if [[ $cleanup == "s" || $cleanup == "S" ]]; then
    print_step "Limpiando imagenes Docker antiguas..."
...
echo ""
print_step "        Construyendo todas las imagenes Docker..."
...
print_step "Construyendo todas las imagenes..."
print_info "Este proceso puede tomar varios minutos la primera vez"

if docker-compose build; then
    print_success "Todas las imagenes construidas correctamente"
else
    print_error "Error durante la construccion de imagenes"
...
print_step "Esperando inicializacion de los servicios..."
sleep 10
...
while [ $attempt -le $max_attempts ]; do
    ...
    if [ $healthy_services -eq $total_services ]; then
        print_success "     ¡Todos los servicios estan saludables!"
        echo ""
        break
    fi
...
echo -e "â€¢ Detener app: docker-compose down"
echo -e "â€¢ Reiniciar: docker-compose restart"
echo -e "â€¢ Ver estado: docker-compose ps"
echo ""
echo -e "${CYAN}      LOGS POR MICROSERVICIO:${NC}"
echo -e "â€¢ Logs MySQL:        docker-compose logs -f db"
echo -e "â€¢ Logs API Flask:    docker-compose logs -f api"
echo -e "â€¢ Logs Frontend:     docker-compose logs -f web"
echo -e "â€¢ Logs FastAPI:      docker-compose logs -f notifications"
echo -e "â€¢ Logs Nginx:        docker-compose logs -f nginx"
echo ""
...
# FunciÃ³n para verificar health segÃºn el tipo de servicio
check_service_health() {
    ...
    print_error "$service_name: No pudo verificarse despuÃ©s de $max_retries intentos"
    return 1
}

echo ""
print_success "DESPLIEGUE COMPLETADO"
print_info "AplicaciÃ³n disponible en: http://localhost"
...
echo ""
echo -e "${GREEN}     Â¡La aplicaciÃ³n TODO estÃ¡ lista para usar!${NC}"
echo ""